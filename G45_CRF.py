...
# ==============================================================================
# 新增依赖：密集条件随机场 (Dense CRF)
# 如果运行报错，请在终端执行：pip install pydensecrf
# ==============================================================================
try:
    import pydensecrf.densecrf as dcrf
    from pydensecrf.utils import unary_from_softmax, create_pairwise_bilateral, create_pairwise_gaussian
except ImportError:
    raise ImportError("请先安装 pydensecrf 库: pip install pydensecrf")
...
# ==============================================================================
# 新增：Dense CRF 核心后处理算法
# ==============================================================================
def apply_dense_crf(original_image, prob_map, iter_num=3):  # 减少迭代次数到 3
    """
    调整了参数的温和版 Dense CRF
    """
    H, W = prob_map.shape

    # 为了保护微弱的预测信号，可以在传入 CRF 前稍微提升一下对比度 (可选)
    # prob_map = np.power(prob_map, 0.8)

    prob_map = np.clip(prob_map, 1e-5, 1.0 - 1e-5)
    probs = np.stack([1.0 - prob_map, prob_map], axis=0)
    U = unary_from_softmax(probs)

    d = dcrf.DenseCRF2D(W, H, 2)
    d.setUnaryEnergy(U)

    # 降低纯空间平滑的惩罚力度 (compat 3 -> 1)
    d.addPairwiseGaussian(sxy=(3, 3), compat=1, kernel=dcrf.DIAG_KERNEL, normalization=dcrf.NORMALIZE_SYMMETRIC)

    # 缩小颜色容忍度，降低双边滤波权重 (srgb 15 -> 5, compat 10 -> 4)
    # sxy 保持不变，因为高分辨率图像需要一定的空间跨度来感知邻域
    d.addPairwiseBilateral(sxy=(40, 40), srgb=(5, 5, 5), rgbim=original_image,
                           compat=4, kernel=dcrf.DIAG_KERNEL, normalization=dcrf.NORMALIZE_SYMMETRIC)

    Q = d.inference(iter_num)
    crf_prob = np.array(Q)[1, :].reshape((H, W))
...
def visualize_custom_hubei(
    ...
    save_dir = os.path.join("results", "G45_CRF")
    ...
    for idx, file in enumerate(tqdm(file_names, desc="Saving Images")):
        ...
        # ----------------------------------------------------------------------
        # 新增：在此处应用 Dense CRF
        # CRF 通常需要原始图像在 RGB 色彩空间下进行颜色势的计算
        img_rgb = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)

        # 应用 Dense CRF
        pred = apply_dense_crf(img_rgb, pred.astype(np.float32), iter_num=5)
        # ----------------------------------------------------------------------
